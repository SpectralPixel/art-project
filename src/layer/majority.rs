...
pub fn calculate_next_gen(cur_gen: &[Pixel]) -> [f32; ARRAY_LENGTH] {
    ...
    for cell_index in 0..cur_gen.len() {
        ...
        let cell_sum = super::calc_cell_sum(&cell_pos, pattern(), &cur_gen, CellMode::Channel(ColorChannel::Blue));

        let calculated_cell = apply_rules(cell_sum as u32, cur_cell_value.as_color().b());
...
fn apply_rules(value: u32, cur_cell: f32) -> f32 {
    if (cur_cell >= 0.95 && (value >= 40 && value <= 80)) || (cur_cell < 0.95 && (value >= 41 && value <= 81)) {
        1.
    } else {
        0.
    }
