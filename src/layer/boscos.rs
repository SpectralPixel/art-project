...
pub fn calculate_next_gen(cur_gen: &[Pixel]) -> [f32; ARRAY_LENGTH] {
    ...
    for cell_index in 0..cur_gen.len() {
        ...
        let cell_sum = super::calc_cell_sum(&cell_pos, pattern(), &cur_gen, CellMode::Channel(ColorChannel::Red, SURVIVAL_VALUE));

        let calculated_cell = apply_rules(cell_sum, super::round_color_fix(cur_cell_value.as_color()).r());
...
fn apply_rules(value: f32, cur_cell: f32) -> f32 {
    if (cur_cell >= SURVIVAL_VALUE && (value >= 33. && value <= 57.)) || (cur_cell < SURVIVAL_VALUE && (value >= 34. && value <= 45.)) {
        1.
    } else {
